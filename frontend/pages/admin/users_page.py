...
from frontend.dialogs.password_reset_dialog import PasswordResetDialog
...
class UsersPage(QWidget):
    ...
    def _build_action_buttons(self, user: User) -> QWidget:
        ...
        btn_reset = QPushButton("Şifre")
        btn_reset.setFixedSize(64, 36)
        btn_reset.setCursor(Qt.CursorShape.PointingHandCursor)
        btn_reset.setToolTip("Şifre sıfırla")
        btn_reset.setStyleSheet(self._btn_style("#3B82F6", "#1E3A8A"))
    ...
    def _reset_password(self, user: User) -> None:
        dlg = PasswordResetDialog(self._ctx, user, parent=self)
        if dlg.exec() and dlg.new_password:
            Toast.show_success(
                self,
                f"{user.username} için yeni şifre: {dlg.new_password}  (panoya kopyalandı)",
            )
