...
class DatabaseManager:
    ...
    def _create_schema(self) -> None:
        self._conn.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS saved_cards (
                id          INTEGER PRIMARY KEY AUTOINCREMENT,
                user_id     INTEGER NOT NULL,
                title       TEXT,                            -- Kullanıcının verdiği isim
                holder      TEXT    NOT NULL,                -- Kart üzerindeki isim
                bin         TEXT    NOT NULL,                -- İlk 6 hane
                last4       TEXT    NOT NULL,                -- Son 4 hane
                brand       TEXT    NOT NULL DEFAULT 'unknown',
                expiry_mm   INTEGER NOT NULL CHECK(expiry_mm BETWEEN 1 AND 12),
                expiry_yy   INTEGER NOT NULL CHECK(expiry_yy BETWEEN 0 AND 99),
    ...
    def _migrate(self) -> None:
        """
        Idempotent şema güncellemeleri. Eski DB'lere yeni alanlar ekler.
        Mevcut veriler korunur.
        """
        existing_cols = {
            r[1] for r in self._conn.execute("PRAGMA table_info(orders)").fetchall()
        }
        adds = [
            ("payment_method",  "TEXT"),
        ...
        for col, ddl in adds:
            if col not in existing_cols:
                ...
                logger.info("orders tablosuna kolon eklendi: %s", col)
        # order_number için unique index (NULL'lar tekrar edebilir → kısmî index)
        self._conn.execute(
