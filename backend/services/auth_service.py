...
class AuthService:
    ...
    def admin_reset_password(self, user_id: int, new_password: str) -> bool:
        """
        Admin yetkisiyle bir kullanıcının şifresini sıfırlar.
        Eski şifre kontrolü yoktur — yalnızca admin panelinden çağrılmalıdır.
        Şifre minimum 4 karakter olmalıdır (admin tarafı gevşek kuralla).
        """
        new_password = (new_password or "").strip()
        if len(new_password) < 4:
            raise ValueError("Şifre en az 4 karakter olmalıdır.")

        ok = self._repo.update_password(user_id, self._hash_password(new_password))
        if ok:
            logger.info("Admin şifre sıfırladı: user_id=%d", user_id)
        return ok
